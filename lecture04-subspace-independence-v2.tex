...
\vfill
Understanding both languages helps solve $Ax = b$:
\begin{itemize}
\item \x{Question}: Is $b$ in column space? (verification)
\item \x{Answer}: Solve $Ax = b$ (finding parameters)
\end{itemize}

They're \x{dual} views of the same problem!
\end{frame}
...
Since $U$ has linearly independent columns, each column of $P - Q$ must be zero.
...
\begin{frame}[fragile]{Example: Pivot Structure Creates Independence}
Recall cross-filling:
...
\textbf{Observe}:
\begin{itemize}
\item $\mathbf{u}_1$ came from pivot at row 1
\item After peeling $\mathbf{u}_1$, row 1 became zero
\item $\mathbf{u}_2$ has $\mathbf{u}_2[\text{row } 1] = 0$ (first entry is 0)
\end{itemize}

This pattern forces independence!
\end{frame}
