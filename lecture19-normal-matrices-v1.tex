...
\begin{frame}[shrink=6]{Why the Reverse Order Is Identity}

The old cross-filling tool gives

$$
\rd{P}=\bl{U}\bl{U}^H
\qquad\text{and the columns of }\bl{U}\text{ are linearly independent.}
$$
...
Substitute $\rd{P}=\bl{U}\bl{U}^H$:

$$
(\bl{U}\bl{U}^H)(\bl{U}\bl{U}^H)=\bl{U}\bl{U}^H.
$$

\vfill\pause

Left-cancel $\bl{U}$ and right-cancel $\bl{U}^H$:

$$
\bl{U}^H\bl{U}=I_r.
$$

\x{This is the AB/BA mechanism from the projection lectures.}

\end{frame}


\begin{frame}[shrink=6]{Read the Reverse Order as a Table}

The reversed product is the inner-product table of the columns:
...
\x{So cross-filling produces an orthonormal column table.}
...
\begin{frame}[shrink=6]{Reverse the Order Again}
...
Use the previous projection-lecture fact.  If

$$
\bl{A}\rd{B}=I,
$$

then the reversed product is first a projection:

$$
(\rd{B}\bl{A})^2
=\rd{B}\underbrace{\bl{A}\rd{B}}_{I}\bl{A}
=\rd{B}\bl{A}.
$$

\vfill

So $\rd{B}\bl{A}$ is a projection.

\end{frame}


\begin{frame}[shrink=6]{Why That Projection Is Identity}

Continue with $\bl{A}\rd{B}=I$ and $Q=\rd{B}\bl{A}$.

\vfill

We already know $Q$ is a projection.

\vfill\pause

Trace reverses products:

$$
\operatorname{tr}(Q)
=\operatorname{tr}(\rd{B}\bl{A})
=\operatorname{tr}(\bl{A}\rd{B})
=\operatorname{tr}(I)
=n.
$$
...
Now use the projection $I-Q$:

$$
I-Q \quad\text{is also a projection.}
$$
...
$$
\operatorname{tr}(I-Q)=n-n=0.
$$

\vfill

For a projection, trace equals rank:
...
\begin{frame}[shrink=6]{Apply the Reversal to $\Omega$}
...
the reversed order is also identity:
