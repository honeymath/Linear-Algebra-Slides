...
\begin{tikzpicture}[scale=0.8]
\matrix (m) [matrix of math nodes, left delimiter=(, right delimiter=), ampersand replacement=\&] {
|[fill=red!20]| 1 \& |[fill=red!20]| 2 \& |[fill=red!20]| 1 \& \vrule \& |[fill=red!20]| 4 \\
|[fill=red!20]| 2 \& |[fill=red!20]| 4 \& |[fill=red!20]| 2 \& \vrule \& |[fill=red!20]| 8 \\
|[fill=red!20]| 3 \& |[fill=red!20]| 6 \& |[fill=red!20]| 3 \& \vrule \& |[fill=red!20]| 12 \\
};
\node[right=0.5cm of m] {$(R_1 \mid \mathbf{b}_1)$};
\end{tikzpicture}
\end{center}
...
\begin{frame}{Step 4: Equations from $(R_1 \mid \mathbf{b}_1)$}

Each row of $(R_1 \mid \mathbf{b}_1)$ represents an equation:

\vspace{0.5em}

\begin{align*}
\text{Row 1}: &\quad 1x_1 + 2x_2 + 1x_3 = 4 \quad \text{(this is \x{eq1})} \\
\text{Row 2}: &\quad 2x_1 + 4x_2 + 2x_3 = 8 \quad \text{(this is \x{2·eq1})} \\
\text{Row 3}: &\quad 3x_1 + 6x_2 + 3x_3 = 12 \quad \text{(this is \x{3·eq1})}
\end{align*}

\vspace{1em}

\x{Key observation}: All three rows represent the \textbf{same equation} (scaled versions of eq1).

\vspace{0.5em}

So $(R_1 \mid \mathbf{b}_1)$ contains only \x{one independent equation}: eq1.

\end{frame}

\begin{frame}[fragile]{Step 5: Compute Remainder}

$$\text{Remainder} = (A \mid \mathbf{b}) - (R_1 \mid \mathbf{b}_1)$$
...
\begin{tikzpicture}[scale=0.7]
\node at (0, 0.5) {$=$};
\matrix (m) [right=0.5cm, matrix of math nodes, left delimiter=(, right delimiter=), ampersand replacement=\&] {
0 \& 0 \& 0 \& \vrule \& 0 \\
0 \& 0 \& -1 \& \vrule \& 1 \\
0 \& 0 \& -1 \& \vrule \& 1 \\
};
\node[right=0.5cm of m] {Remainder};
\end{tikzpicture}
\end{center}
...
Each row of Remainder represents an equation:

\vspace{0.5em}

\begin{align*}
\text{Row 1}: &\quad 0 = 0 \quad \text{(\x{trivial}, no information)} \\
\text{Row 2}: &\quad 0x_1 + 0x_2 - 1x_3 = 1 \\
\text{Row 3}: &\quad 0x_1 + 0x_2 - 1x_3 = 1
\end{align*}

\vspace{0.5em}

\x{What are these equations in terms of the original system?}

\vspace{0.5em}

\begin{align*}
\text{Row 2}: &\quad (\text{eq2}) - 2 \cdot (\text{eq1}) = -x_3 = 1 \\
\text{Row 3}: &\quad (\text{eq3}) - 3 \cdot (\text{eq1}) = -x_3 = 1
\end{align*}

\vspace{0.5em}

Both rows represent the \x{same equation}: $-x_3 = 1$, or $x_3 = -1$.
...
\begin{frame}[fragile]{Step 9: Second Rank-One Piece $(R_2 \mid \mathbf{b}_2)$}

$$(R_2 \mid \mathbf{b}_2) = \begin{pmatrix} 0 \\ -1 \\ -1 \end{pmatrix} \begin{pmatrix} 0 & 0 & -1 & | & 1 \end{pmatrix}$$
...
\node[right=0.5cm of m] {$(R_2 \mid \mathbf{b}_2)$};
\end{tikzpicture}
\end{center}

\vspace{0.5em}

Note: I've adjusted signs so row 2 reads: $x_3 = -1$.

\end{frame}

\begin{frame}{Step 10: Equations from $(R_2 \mid \mathbf{b}_2)$}

Each row of $(R_2 \mid \mathbf{b}_2)$:

\vspace{0.5em}

\begin{align*}
\text{Row 1}: &\quad 0 = 0 \quad \text{(trivial)} \\
\text{Row 2}: &\quad x_3 = -1 \quad \text{(from eq2 $-$ 2·eq1)} \\
\text{Row 3}: &\quad x_3 = -1 \quad \text{(from eq3 $-$ 3·eq1)}
\end{align*}

\vspace{1em}

Again, all rows represent the \x{same equation}: $x_3 = -1$.

\end{frame}

\begin{frame}[fragile]{Step 11: Final Remainder is Zero}

$$\text{Second Remainder} = \text{First Remainder} - (R_2 \mid \mathbf{b}_2)$$
...
\node[right=0.5cm of m] {All zeros!};
\end{tikzpicture}
\end{center}
...
We've decomposed the augmented matrix into two rank-one pieces.

\end{frame}

\begin{frame}{Step 12: The Simplified Equation System}
...
\hline
$(R_1 \mid \mathbf{b}_1)$ & $x_1 + 2x_2 + x_3 = 4$ \\
\hline
$(R_2 \mid \mathbf{b}_2)$ & $x_3 = -1$ \\
\end{tabular}
\end{center}

\vspace{1em}

\x{This is our simplified system!} Two equations, three unknowns.

\vspace{1em}

Original: 3 equations $\to$ Simplified: 2 independent equations.

\end{frame}

\begin{frame}{Step 13: Backward Solving}
...
\begin{frame}{Step 14: Identify Pivot and Free Variables}
...
\begin{frame}{Step 15: General Solution}
...
\begin{frame}{Step 16: Decompose into Particular + Null Space}
...
\begin{frame}{Step 17: Verification (Particular Solution)}
...
\begin{frame}{Step 17: Verification (Null Space Vector)}
...
Cross-filling $(A \mid \mathbf{b})$ can produce two outcomes:

\vspace{1em}

\x{Case 1}: All remainder rows eventually become zero.
\begin{itemize}
\item No contradiction
\item Solution \x{exists}
\item Example: our $3 \times 3$ example
\end{itemize}

\vspace{1em}

\x{Case 2}: Some remainder row becomes $\begin{pmatrix} 0 & 0 & \cdots & 0 & | & c \end{pmatrix}$ where $c \neq 0$.
\begin{itemize}
\item This represents equation $0 = c$ (contradiction!)
