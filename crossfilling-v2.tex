...
\vfill
\textbf{Notice}: Both products use \x{the same ratio}!
\begin{itemize}
...
\vfill
They're using the same base ingredient mix, just different amounts!
\end{frame}

\begin{frame}{Finding the Hidden Product}
If both use the same ratio, there must be a hidden intermediate product:

$$\text{Let's call it \tea~(tea) = 2\leaf~+ 1\lemon}$$

\vfill
Then:
\begin{itemize}
\item \bento~needs: 1\tea~+ (other stuff)
\item \ramen~needs: 1\tea~+ (other stuff)
\end{itemize}

\vfill
\x{This is a "rank-one pattern"} — one base ingredient mix used in different quantities.
\end{frame}
...
\vfill
\textbf{Interpretation}: This is the \x{\tea~pattern}!
\begin{itemize}
\item Make \tea: 2\leaf~+ 1\lemon~+ 0\bean~+ 2\cow
\item \bento~uses: 1\tea
\item \ramen~uses: 1\tea
\end{itemize}
\end{frame}

\begin{frame}{Insight: What is This Pattern?}
The pattern $R_1$ represents a \x{hidden intermediate product}:

$$\text{\tea} = \begin{pmatrix} 2 \\ 1 \\ 0 \\ 2 \end{pmatrix}
\begin{pmatrix} 1 & 1 \end{pmatrix}$$

\vspace{0.3cm}
- \x{Column} $(2,1,0,2)$: recipe to make \tea~from raw materials
- \x{Row} $(1,1)$: how much \tea~each final product needs

\vspace{0.3cm}
Key: \x{One pattern, repeated!} This is a "rank-one" table.
\end{frame}
...
\vfill
Key: The \x{entire cross is now zero}!
- All of \leaf~row: \rem{zero}
- All of \bento~column: \rem{zero}
\end{frame}
...
$$C_2 = \begin{array}{c|cc}
 & \text{\bento} & \text{\ramen} \\
\hline
\text{\bean} & \rem{0} & \pvt{4} \\
\text{\cow} & \rem{0} & -1
\end{array}$$
\end{frame}

\begin{frame}{Calculation: Fill Second Cross}
Cross: column $(0,0,0,0)^T$ and row $(0,4)$
...
\vfill
\textbf{Interpretation}: This is pure \coffee~(made from 4\bean) used only in \ramen.
\end{frame}

\begin{frame}{Result: What's Left?}
$$C_3 = C_2 - R_2 = \begin{array}{c|cc}
 & \text{\bento} & \text{\ramen} \\
\hline
\text{\leaf} & 0 & 0 \\
\text{\lemon} & 0 & 0 \\
...
\begin{frame}{Insight: Third Pattern}
$$R_3 = \begin{array}{c|cc}
 & \text{\bento} & \text{\ramen} \\
\hline
\text{\leaf} & 0 & 0 \\
\text{\lemon} & 0 & 0 \\
...
\vfill
\textbf{Interpretation}: Adjustment pattern — \ramen~needs 1 \x{less} \cow~than what \tea~provides.

\vfill
Final remainder: $C_3 - R_3 = $ zero table. \x{Done!}
\end{frame}
...
\vfill
Three \x{simple patterns} (rank-one) = One \x{complex table}!
\end{frame}

\begin{frame}{Insight: The Hidden Production Chain}
\textbf{What we found:}

$$\text{Raw materials} \xrightarrow{A}
\begin{cases}
\text{\tea} \\
\text{\coffee} \\
\text{adjustment}
\end{cases}
\xrightarrow{B} \text{Final products}$$

\vfill
Instead of direct recipes ($C$), we found:
\begin{itemize}
\item $A$: How to make \tea, \coffee~from raw materials
\item $B$: How much \tea, \coffee~each final product needs
\item $C = AB$: The composition
\end{itemize}

\vfill
Key: \x{Cross-filling discovered the hidden intermediate products!}
\end{frame}
