...
\begin{tikzpicture}[scale=0.8]
% Domain
\draw[thick, blue!50, fill=blue!10] (0,0) ellipse (3cm and 2cm);
\node at (0, 2.5) {Domain of $B$};

% Col(B)
\draw[thick, green!50, fill=green!10] (0.8, 0) ellipse (1.5cm and 1.2cm);
\node[green!50!black] at (0.8, -1.5) {Col$(B)$};

% Null(A)
\draw[thick, red!50, fill=red!10] (-1.2, 0) ellipse (1.2cm and 0.8cm);
\node[red!50!black] at (-1.2, -1.2) {Null$(A)$};

% Zero vector
\fill (0,0) circle (2pt);
\node[below] at (0,-0.3) {$\mathbf{0}$};

% Arrow showing B maps
\draw[->, very thick] (3.5, 0) -- node[above] {$B$} (5, 0);

% Codomain
\draw[thick] (6.5, 0) ellipse (2cm and 2cm);
\node at (6.5, 2.5) {Codomain};

% Show Col(B) image
\draw[thick, green!50, fill=green!20] (6.5, 0.5) ellipse (1cm and 1.2cm);
\node[green!50!black] at (6.5, -1.2) {Image};

% Null(A) region
\draw[thick, red!50, fill=red!20] (6.5, -0.8) ellipse (0.8cm and 0.5cm);
\node[red!50!black] at (8.5, -0.8) {$\leftarrow$ Null$(A)$?};
...
\x{Question}: Does any nonzero vector from Col$(B)$ land in Null$(A)$?

If \x{NO}: $\text{Null}(AB) = \text{Null}(B)$ (no new dependencies created)
