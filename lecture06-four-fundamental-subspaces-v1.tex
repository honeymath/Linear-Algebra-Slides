...
\draw[thick, blue!30, fill=blue!5] (-4,-2) rectangle (0,2);
\node at (-2, 2.4) {\small Domain of $B$ ($\mathbb{R}^q$)};
\node at (-2, -2.4) {\small Contains Null$(B)$, Null$(AB)$};

% Middle: Codomain of B = Domain of A (R^n)
\draw[thick, purple!40, fill=purple!5] (1.5,-2.5) rectangle (6.5,2.5);
\node at (4, 3) {\textbf{Codomain of $B$ = Domain of $A$}};
\node at (4, 2.6) {($\mathbb{R}^n$)};

% Col(B) in the middle space
\draw[thick, green!60, fill=green!15] (2.5, 0) ellipse (1.2cm and 1.5cm);
\node[green!50!black] at (2.5, -1.8) {Col$(B)$};

% Null(A) in the middle space
\draw[thick, red!60, fill=red!15] (5, 0) ellipse (1cm and 1.3cm);
\node[red!50!black] at (5, -1.6) {Null$(A)$};

% Zero vector at intersection
\fill (3.8, 0) circle (2pt);
\node[below] at (3.8, -0.3) {\small $\mathbf{0}$};

% Right: Codomain of A (R^m)
\draw[thick, blue!30, fill=blue!5] (8,-2) rectangle (12,2);
\node at (10, 2.4) {\small Codomain of $A$ ($\mathbb{R}^m$)};

% Arrows
\draw[->, ultra thick] (0.5, 0) -- node[above] {$B$} (1.2, 0);
\draw[->, ultra thick] (6.8, 0) -- node[above] {$A$} (7.7, 0);

% Question annotation
\node[purple!70!black, align=center] at (4, -3.2) {\textbf{Question:} Do Col$(B)$ and Null$(A)$ intersect?};
\end{tikzpicture}
\end{center}

\x{Key}: Col$(B)$ and Null$(A)$ both live in the \x{same space} ($\mathbb{R}^n$)!

If they only meet at $\mathbf{0}$: $\text{Null}(AB) = \text{Null}(B)$ (no new dependencies)
