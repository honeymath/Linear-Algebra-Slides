...
\x{Each row represents an equation}:
\begin{align*}
\text{Row 1}: &\quad 0 = 0 \quad \text{(pivot row: absorbed into $R_1$)} \\
\text{Row 2}: &\quad -x_3 = 1 \quad \text{(eq2 $-$ 2$\cdot$eq1)} \\
\text{Row 3}: &\quad -x_3 = 1 \quad \text{(eq3 $-$ 3$\cdot$eq1)}
\end{align*}

\x{Observation}: Column 1 is eliminated (pivot column).
Rows 2 and 3 now involve \x{fewer variables} than the original equations.
...
\x{Each row represents an equation}:
\begin{align*}
\text{Row 1}: &\quad 0 = 0 \quad \text{(pivot row of $R_1$: already absorbed)} \\
\text{Row 2}: &\quad -x_3 = 1 \quad \text{(pivot row of $R_2$)} \\
\text{Row 3}: &\quad -x_3 = 1 \quad \text{(same equation, rows 2 \& 3 are proportional)}
\end{align*}

\x{Key}: Only \x{one independent equation}: $x_3 = -1$.

\end{frame}

\begin{frame}{Step 11: Why the Simplified System is Equivalent}

\x{We showed}: $(A \mid \mathbf{b}) = (R_1 \mid \mathbf{b}_1) + (R_2 \mid \mathbf{b}_2)$

\vspace{0.5em}

Each $R_i$ is rank-one: all rows are multiples of the \textbf{pivot row}.

The pivot rows of $R_1$ and $R_2$ form a \x{basis for the row space} of $(A \mid \mathbf{b})$.

\vspace{0.5em}

\x{Therefore}: Every original equation is a linear combination of the simplified equations. The two systems have \textbf{exactly the same solution set}.

\vspace{1em}

\x{Simplified system} (one equation per piece):
\begin{align*}
\text{From } R_1: \quad &x_1 + 2x_2 + x_3 = 4 \\
\text{From } R_2: \quad &-x_3 = 1 \quad \text{(i.e., } x_3 = -1\text{)}
\end{align*}

\end{frame}

\begin{frame}{Step 11 (continued): Pivot and Free Variables}

Each cross-filling eliminates \x{at least one column} (the pivot column).

The variable at the pivot column is determined by the equation $\to$ \x{pivot variable}.

Variables at non-pivot columns remain undetermined $\to$ \x{free variables}.
...
\begin{frame}{Step 13: General Solution}
...
\begin{frame}{Step 14: Decompose into Particular + Null Space}
...
\begin{frame}{Step 15: Verification (Particular Solution)}
...
\begin{frame}{Step 15: Verification (Null Space Vector) (continued)}
...
\x{Key insight}: The pivot rows of the rank-one pieces form an \textbf{equivalent simpler system} (same row space as the original).
...
$$\begin{pmatrix} \alert{0} & 0 & \alert{0} & | & \boxed{1} \end{pmatrix}$$
...
\begin{frame}{Why Do These Vectors Form a Basis?}

Suppose solving $A\mathbf{x} = \mathbf{0}$ with $k$ free variables gives:
$$\mathbf{x} = t_1 \mathbf{u}_1 + t_2 \mathbf{u}_2 + \cdots + t_k \mathbf{u}_k$$

\begin{prop}
$\mathbf{u}_1, \ldots, \mathbf{u}_k$ form a \x{basis} for $\text{Null}(A)$.
\end{prop}

\x{Proof (spans)}: Let $\mathbf{w} \in \text{Null}(A)$. Then $\mathbf{w}$ satisfies $A\mathbf{x} = \mathbf{0}$, so $\mathbf{w}$ must have the form $t_1 \mathbf{u}_1 + \cdots + t_k \mathbf{u}_k$ for some $t_1, \ldots, t_k$ (since cross-filling finds \textbf{all} solutions).

\end{frame}

\begin{frame}{Why Do These Vectors Form a Basis? (continued)}

\x{Proof (linearly independent)}: Each $\mathbf{u}_i$ is constructed by setting the $i$-th free variable to 1 and all other free variables to 0.

\vspace{0.5em}

So in the free variable positions:
$$\mathbf{u}_i \text{ has } \begin{cases} 1 & \text{in free variable } i \\ 0 & \text{in all other free variables} \end{cases}$$

\vspace{0.5em}

If $c_1 \mathbf{u}_1 + \cdots + c_k \mathbf{u}_k = \mathbf{0}$, look at the $i$-th free variable position:
$$c_i \cdot 1 + \sum_{j \neq i} c_j \cdot 0 = 0 \quad \Rightarrow \quad c_i = 0$$

This holds for all $i$, so $\mathbf{u}_1, \ldots, \mathbf{u}_k$ are linearly independent. $\square$
...
\item Pivot rows form equivalent simplified system (same row space)
\item Each cross-filling eliminates at least one variable (pivot variable)
\item Solve backwards from simplest equation; remaining variables are free
\end{enumerate}
