...
\begin{frame}{The Dream: Projections Make Powers Trivial}
...
% ADR: Connect eigenvalues to L14's characteristic polynomial.
% Introduce the RUNNING EXAMPLE that will carry through §3-4.
% Clean eigenvalues 0,1,2 make all later computations integer arithmetic.
% AUDIENCE EMOTION: confidence — "I already know how to find eigenvalues"
% AUDIENCE ATTENTION: the running example A is introduced once and reused
% throughout — no context-switching cost in later frames.
\begin{frame}{Where Do Eigenvalues Come From?}

From Lecture 14: the \x{characteristic polynomial}

$$\det(tI - A) = (t - \lambda_1)(t - \lambda_2)\cdots(t - \lambda_n)$$

\vfill

The roots $\lambda_1, \ldots, \lambda_n$ are the \x{eigenvalues}.
...
$R(t)$ has degree $< 3$, so it is determined by \x{three values}:
...
% ADR: The 3-point case uses the RUNNING EXAMPLE's eigenvalues 0,1,2.
% The audience should notice: "wait, those are the eigenvalues of our
% matrix A!" — this foreshadows the connection in §4 without us saying it.
% We let the audience make the connection themselves (discovery > exposition).
%
% The general pattern formula at the bottom is stated as an OBSERVATION,
% not a theorem — it emerges from the three cases the audience has seen.
% AUDIENCE EMOTION: anticipation — "these are A's eigenvalues... is that
% a coincidence?" (the connection to §4 is brewing in their minds)
% AUDIENCE ATTENTION: the 3×3 table with 1/0 pattern is visually
% satisfying — the identity matrix pattern is now unmistakable.
\begin{frame}[shrink=5]{$m = 3$: Three Points — at $0, 1, 2$}

Dividing $g(t)$ by $t(t-1)(t-2)$: remainder has degree $\leq 2$,

determined by \x{three} values: $g(0)$, $g(1)$, $g(2)$.
...
\begin{tabular}{c|ccc}
 & $\underbrace{\dfrac{(t-1)(t-2)}{(0-1)(0-2)}}_{f_0(t) = \frac{(t-1)(t-2)}{2}}$
 & $\underbrace{\dfrac{t(t-2)}{(1-0)(1-2)}}_{f_1(t) = -t(t-2)}$
 & $\underbrace{\dfrac{t(t-1)}{(2-0)(2-1)}}_{f_2(t) = \frac{t(t-1)}{2}}$ \\[4pt]
\hline
$t = 0$ & $1$ & $0$ & $0$ \\
$t = 1$ & $0$ & $1$ & $0$ \\
$t = 2$ & $0$ & $0$ & $1$
...
Pattern: $f_{\lambda_i}(t) = \dfrac{\text{product of all }(t - \lambda_j)\text{ for }j \neq i}{\text{same product evaluated at }\lambda_i}$

\end{frame}


% ADR: General theorem + progression table. By now the audience has seen
% three cases. The theorem just NAMES what they already know.
% The progression table (m=1, m=2, general m) shows the pattern explicitly.
% The closing line "Remainder Theorem is the special case m=1" is the
% narrative punchline: everything in this section was one idea at different scales.
%
% AUDIENCE EMOTION: satisfaction — "I see the whole picture now"
% AUDIENCE ATTENTION: the progression table organizes three slides
% of information into one scannable visual — compression is satisfying.
...
\begin{frame}{Arithmetic in Value Tables}

\x{Addition}: value tables add entry-wise.

$$\underbrace{(0,1,2)}_{A} + \underbrace{(1,1,1)}_{I} = \underbrace{(1,2,3)}_{A + I} \qquad \checkmark$$

\vfill\pause

\x{Multiplication}: value tables \x{multiply entry-wise}!

$$\underbrace{(0,1,2)}_{A} \cdot \underbrace{(0,1,2)}_{A} = \underbrace{(0,1,4)}_{A^2} \qquad \checkmark$$
...
Matrix multiplication (hard) $\longleftrightarrow$ entry-wise multiplication (easy).

\vfill

The value table turns matrix algebra into \x{$\mathbb{R}^3$ arithmetic}.
...
% Three examples, chosen to cover:
%   1. Projection (P₁): value table has only 0s and 1s → P²=P ✓
%   2. Non-projection (A): value table has a 2 → 2²=4≠2 → A²≠A ✗
%   3. Sum of projections (P₀+P₁): value table (1,1,0), still 0s and 1s → projection ✓
%
% The non-projection example is critical: it shows that the test can
% FAIL, which makes the passing cases more meaningful.
% The sum P₀+P₁ example is the surprise: "a sum of projections is
% also a projection?!" — the value table makes it obvious.
%
...
\x{Is $P_0 + P_1$ a projection?} \; Value table: $(1,1,0) + (0,1,0)$\ldots

\medskip

Wait --- that gives $(1, \rd{2}, 0)$, and $2^2 = 4 \neq 2$. \; Not a projection.

\vfill\pause

\x{Is $P_0 + P_2$ a projection?} \; Value table: $(1,0,0) + (0,0,1) = (1,0,1)$.

Square: $(1,0,1)$. \; Same! \; $(P_0 + P_2)^2 = P_0 + P_2$ \;\checkmark
...
% ADR: Prove (A - λᵢI)Pᵢ = 0 via VALUE TABLES, not by Lagrange definition.
% The value table argument: (t - λᵢ) is zero at λᵢ, fᵢ is zero everywhere
% EXCEPT at λᵢ. Their product is zero at every eigenvalue → matrix = 0.
% This is the value table method doing EVERYTHING — the user's explicit
% request: "as long as you have the value table method, use it for everything."
%
% AUDIENCE EMOTION: "the value table even gives us eigenvectors!"
% AUDIENCE ATTENTION: the three checks are fast and visual.
\begin{frame}{$(A - \lambda_i I) \cdot P_{\lambda_i} = 0$ --- by Value Tables}

\x{$\lambda_1 = 0$}: \; value table of $(A - 0 \cdot I) = A$ is $(0, 1, 2)$.

$(0,1,2) \cdot \underbrace{(1,0,0)}_{P_0} = (0, 0, 0) = 0$ \;\checkmark

\vfill\pause

\x{$\lambda_2 = 1$}: \; value table of $(A - I)$ is $(0\!-\!1,\; 1\!-\!1,\; 2\!-\!1) = (-1, 0, 1)$.

$(-1,0,1) \cdot \underbrace{(0,1,0)}_{P_1} = (0, 0, 0) = 0$ \;\checkmark

\vfill\pause

\x{$\lambda_3 = 2$}: \; value table of $(A - 2I)$ is $(-2, -1, 0)$.

$(-2,-1,0) \cdot \underbrace{(0,0,1)}_{P_2} = (0, 0, 0) = 0$ \;\checkmark

\vfill

\x{Why it works}: $(t - \lambda_i)$ is zero at $\lambda_i$, \; $P_{\lambda_i}$ is zero everywhere \x{else}.

Their product is zero at \x{every} eigenvalue $\implies$ $(A - \lambda_i I)\, P_{\lambda_i} = 0$.

\end{frame}


% ADR: Eigenvectors as COLUMNS of projections — no equation solving.
% The previous frame proved (A-λᵢI)Pᵢ = 0 via value tables.
% Now we just rewrite that as APᵢ = λᵢPᵢ and read off columns.
%
...
We just showed: $(A - \lambda_i I)\, P_{\lambda_i} = 0$, \; i.e.\ $A\, P_{\lambda_i} = \lambda_i\, P_{\lambda_i}$.
...
\begin{frame}{Step 3: $A^n$ by Value Tables}
