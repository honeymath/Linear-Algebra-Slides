...
% Null(AB) in left space (OUTER circle)
\draw[thick, orange!60, fill=orange!20] (-1.35, 0) ellipse (0.9cm and 1.1cm);
\node[orange!50!black, font=\tiny] at (-1.35, -1.4) {Null$(AB)$};

% Null(B) in left space (INNER circle, contained in Null(AB))
\draw[thick, orange!80, fill=orange!40] (-1.35, 0.2) ellipse (0.5cm and 0.6cm);
\node[orange!50!black, font=\tiny] at (-1.35, 0.2) {Null$(B)$};
...
% Col(B) in the middle space (with transparency for intersection)
\draw[thick, green!60, fill=green!30, opacity=0.6] (2.3, 0) ellipse (1.1cm and 1.4cm);
\node[green!50!black, font=\small] at (1.5, 0) {Col$(B)$};

% Null(A) in the middle space (with transparency for intersection)
\draw[thick, red!60, fill=red!30, opacity=0.6] (3.7, 0) ellipse (1.1cm and 1.4cm);
\node[red!50!black, font=\small] at (4.5, 0) {Null$(A)$};

% Mark the intersection region
\node[brown!60!black, font=\scriptsize] at (3, 0.8) {Intersection};

% Zero vector in the intersection
\fill (3, 0) circle (2.5pt);
\node[below, font=\small] at (3, -0.3) {$\mathbf{0}$};

% Right: Codomain of A (R^m)
\draw[thick, blue!30, fill=blue!5] (6.2,-2.5) rectangle (8.5,2.5);
\node at (7.35, 2.8) {\footnotesize $\mathbb{R}^m$};

% Col(A) in right space (OUTER circle)
\draw[thick, cyan!60, fill=cyan!20] (7.35, 0) ellipse (0.9cm and 1.1cm);
\node[cyan!50!black, font=\tiny] at (7.35, -1.4) {Col$(A)$};

% Col(AB) in right space (INNER circle, contained in Col(A))
\draw[thick, cyan!80, fill=cyan!40] (7.35, 0.2) ellipse (0.5cm and 0.6cm);
\node[cyan!50!black, font=\tiny] at (7.35, 0.2) {Col$(AB)$};
...
Null$(AB) =$ Null$(B)$ $\Longleftrightarrow$ Col$(B) \cap$ Null$(A) = \{\mathbf{0}\}$
...
\begin{frame}{What Is Transpose?}

The \x{transpose} of a matrix is obtained by \x{flipping it along the main diagonal}.

\vspace{1em}

\x{Effect}: Rows become columns, and columns become rows.

\vspace{1em}

\x{Notation}: For matrix $A$, its transpose is written $A^T$.

\vspace{1em}

\x{Example}:
$$A = \begin{pmatrix} 1 & 2 & 3 \\ 4 & 5 & 6 \end{pmatrix} \quad \xrightarrow{\text{transpose}} \quad A^T = \begin{pmatrix} 1 & 4 \\ 2 & 5 \\ 3 & 6 \end{pmatrix}$$

Row 1 of $A$: $(1, 2, 3)$ becomes column 1 of $A^T$

\end{frame}

\begin{frame}[fragile]{Visual: Flipping Along the Diagonal}

\begin{center}
\begin{tikzpicture}[scale=1.0]
% Original matrix A
\matrix (A) [matrix of math nodes, left delimiter=(, right delimiter=),
             nodes={minimum width=1.8em, minimum height=1.8em}] at (0,0) {
1 & 2 & 3 \\
4 & 5 & 6 \\
};

% Draw diagonal line
\draw[red, thick, dashed] (A-1-1.north west) -- (A-2-3.south east);

\node[below=0.8cm of A] {$A$};

% Arrow
\draw[->, ultra thick, blue] (2.5, 0) -- node[above] {Flip along diagonal} (6.5, 0);

% Transposed matrix A^T
\matrix (AT) [matrix of math nodes, left delimiter=(, right delimiter=),
              nodes={minimum width=1.8em, minimum height=1.8em}] at (10,0) {
1 & 4 \\
2 & 5 \\
3 & 6 \\
};
...
% Draw diagonal line
\draw[red, thick, dashed] (AT-1-1.north west) -- (AT-3-2.south east);

\node[below=0.8cm of AT] {$A^T$};
\end{tikzpicture}
\end{center}

\vspace{0.5em}

\x{Key}: Entry $(i,j)$ of $A$ becomes entry $(j,i)$ of $A^T$.

\end{frame}

\begin{frame}{More Examples}

\x{Example 1}: Square matrix
$$A = \begin{pmatrix} 1 & 2 \\ 3 & 4 \end{pmatrix} \quad \Rightarrow \quad A^T = \begin{pmatrix} 1 & 3 \\ 2 & 4 \end{pmatrix}$$

\vspace{1em}

\x{Example 2}: Tall matrix $\to$ wide matrix
$$A = \begin{pmatrix} 1 & 2 \\ 3 & 4 \\ 5 & 6 \end{pmatrix} \quad \Rightarrow \quad A^T = \begin{pmatrix} 1 & 3 & 5 \\ 2 & 4 & 6 \end{pmatrix}$$

\vspace{1em}

\x{Example 3}: Column vector $\to$ row vector
...
The matrix looks the same when transposed!
\end{defi}
...
\begin{frame}{Summary: Transpose Properties}

\textbf{What we've established:}

\begin{itemize}
\item \x{Definition}: $A^T$ is obtained by flipping $A$ along the main diagonal
\item \x{Product rule}: $(AB)^T = B^T A^T$ (order reverses)
\item \x{Inverse rule}: $(A^{-1})^T = (A^T)^{-1}$
\item \x{Linearity}: $(A + B)^T = A^T + B^T$
\item \x{Symmetric matrices}: $AA^T$ and $A^T A$ are always symmetric
\end{itemize}

\vspace{1em}

These properties apply to \x{all matrices}, and we'll use them to discover deeper relationships between the four fundamental subspaces.

\vspace{1em}

\x{Next}: Orthogonality relationships between subspaces.
