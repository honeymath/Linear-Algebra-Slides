...
Recall: $(A \mid \mathbf{b}) = UV$ where rows of $V$ come from each $R_i$.
...
\begin{itemize}
\item Row 1 from $R_1$: $x_1 + 2x_2 + 0x_3 + x_4 = 5$
\item Row 2 from $R_2$: $0x_1 + 0x_2 + x_3 + 0x_4 = 1$
\end{itemize}
...
From $R_2$ to $R_1$, we went from 1 active variable to 4 active variables.
...
\x{Which one did we choose as pivot in $R_1$?} $x_1$ (at position (1,1))
...
\x{General rule}: From $R_{i+1}$ to $R_i$, new variables appear. The one we chose as pivot = pivot variable. Others = free variables.
...
\begin{align*}
x_3 &= 1 \quad \text{(from } R_2\text{)} \\
x_1 &= 5 - 2(1) - 0 = 3 \quad \text{wait, this gives particular solution!}
\end{align*}
...
From $R_1$ with $\mathbf{b} = \mathbf{0}$: $x_1 + 2x_2 + x_4 = 0$
