...
$R_1 = \bl{\mathbf{w}_1}\bl{\mathbf{w}_1}^T$: \; $B\mathbf{d}_1 = (1,1,1,1,1)^T$, \; $\text{pivot} = q_1 = \pvt{5}$

$\Longrightarrow$ \quad $\boxed{\bl{\mathbf{w}_1} = \dfrac{B\mathbf{d}_1}{\sqrt{q_1}} = \dfrac{1}{\sqrt{5}}(1,1,1,1,1)^T}$

\vspace{0.3em}

$R_2 = \rd{\mathbf{w}_2}\rd{\mathbf{w}_2}^T$: \; $B\mathbf{d}_2 = \tfrac{1}{5}(2, 2, 2, {-}3, {-}3)^T$, \; $\text{pivot} = q_2 = \pvt{\tfrac{6}{5}}$

$\Longrightarrow$ \quad $\boxed{\rd{\mathbf{w}_2} = \dfrac{B\mathbf{d}_2}{\sqrt{q_2}} = \dfrac{1}{\sqrt{30}}(2, 2, 2, {-}3, {-}3)^T}$
...
$R_3 = \gn{\mathbf{w}_3}\gn{\mathbf{w}_3}^T$: \; $\text{col} = (\tfrac{2}{3},\, {-}\tfrac{1}{3},\, {-}\tfrac{1}{3},\, 0,\, 0)^T$, \; $\text{pivot} = \pvt{\tfrac{2}{3}}$

$\Longrightarrow$ \quad $\boxed{\gn{\mathbf{w}_3} = \dfrac{\text{col}}{\sqrt{2/3}} = \dfrac{1}{\sqrt{6}}(2, {-}1, {-}1, 0, 0)^T}$
...
$R_4 = \tl{\mathbf{w}_4}\tl{\mathbf{w}_4}^T$: \; $\text{col} = (0,\, \tfrac{1}{2},\, {-}\tfrac{1}{2},\, 0,\, 0)^T$, \; $\text{pivot} = \pvt{\tfrac{1}{2}}$

$\Longrightarrow$ \quad $\boxed{\tl{\mathbf{w}_4} = \dfrac{\text{col}}{\sqrt{1/2}} = \dfrac{1}{\sqrt{2}}(0, 1, {-}1, 0, 0)^T}$
...
$R_5 = {\color{purple}\mathbf{w}_5}{\color{purple}\mathbf{w}_5}^T$: \; $\text{col} = (0,\, 0,\, 0,\, \tfrac{1}{2},\, {-}\tfrac{1}{2})^T$, \; $\text{pivot} = \pvt{\tfrac{1}{2}}$

$\Longrightarrow$ \quad $\boxed{{\color{purple}\mathbf{w}_5} = \dfrac{\text{col}}{\sqrt{1/2}} = \dfrac{1}{\sqrt{2}}(0, 0, 0, 1, {-}1)^T}$
...
$\bl{\mathbf{w}_1}$ & $\dfrac{1}{\sqrt{5}}(1,1,1,1,1)^T$ & from $B\mathbf{d}_1/\sqrt{q_1}$ \\[8pt]
$\rd{\mathbf{w}_2}$ & $\dfrac{1}{\sqrt{30}}(2,2,2,{-}3,{-}3)^T$ & from $B\mathbf{d}_2/\sqrt{q_2}$ \\[8pt]
$\gn{\mathbf{w}_3}$ & $\dfrac{1}{\sqrt{6}}(2,{-}1,{-}1,0,0)^T$ & from cross-filling $I - P$ \\[8pt]
$\tl{\mathbf{w}_4}$ & $\dfrac{1}{\sqrt{2}}(0,1,{-}1,0,0)^T$ & from cross-filling $I - P$ \\[8pt]
${\color{purple}\mathbf{w}_5}$ & $\dfrac{1}{\sqrt{2}}(0,0,0,1,{-}1)^T$ & from cross-filling $I - P$
\end{tabular}
...
\x{All from cross-filling.} But why are these vectors orthonormal?

\end{frame}


% ADR: AUDIENCE EMOTION — prove orthonormality via U^TU = I.
% P + (I-P) = I gives UU^T = I. For square U, U^TU = I follows.
% Display the matrix product explicitly with colored entries (lecture 8 style).
\begin{frame}{$U^TU = I$ --- Orthonormal Basis of $\mathbb{R}^5$}

$R_j = \mathbf{w}_j\mathbf{w}_j^T$. \quad $\underbrace{R_1 + R_2}_{P} + \underbrace{R_3 + R_4 + R_5}_{I - P} = I$ \quad $\Longrightarrow$ \quad $UU^T = I$

\vspace{0.3em}

$U$ is $5 \times 5$, so $UU^T = I \;\Longrightarrow\; U^TU = I$:

\vspace{0.3em}

\scalebox{0.82}{$\displaystyle
\underbrace{\begin{pmatrix} \bl{\mathbf{w}_1^T} \\[2pt] \rd{\mathbf{w}_2^T} \\[2pt] \gn{\mathbf{w}_3^T} \\[2pt] \tl{\mathbf{w}_4^T} \\[2pt] {\color{purple}\mathbf{w}_5^T} \end{pmatrix}}_{U^T}
\underbrace{\begin{pmatrix} \bl{\mathbf{w}_1} & \rd{\mathbf{w}_2} & \gn{\mathbf{w}_3} & \tl{\mathbf{w}_4} & {\color{purple}\mathbf{w}_5} \end{pmatrix}}_{U}
=
\underbrace{\begin{pmatrix} \bl{1} & 0 & 0 & 0 & 0 \\[2pt] 0 & \rd{1} & 0 & 0 & 0 \\[2pt] 0 & 0 & \gn{1} & 0 & 0 \\[2pt] 0 & 0 & 0 & \tl{1} & 0 \\[2pt] 0 & 0 & 0 & 0 & {\color{purple}1} \end{pmatrix}}_{I_5} \quad\checkmark
$}

\vspace{0.3em}

$\mathbf{w}_i^T\mathbf{w}_j = \delta_{ij}$ \; $\Longrightarrow$ \; $\{\bl{\mathbf{w}_1}, \rd{\mathbf{w}_2}, \gn{\mathbf{w}_3}, \tl{\mathbf{w}_4}, {\color{purple}\mathbf{w}_5}\}$ is \x{orthonormal} $\checkmark$
