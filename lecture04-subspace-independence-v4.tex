...
% Proposition uses yellow title (屎黄色)
...
\newcommand{\soup}{\includegraphics[scale=0.08]{Pictures/soup.png}}
...
\begin{frame}[fragile]{Concrete Example: Rank-3 with Off-Diagonal Pivots}
\x{Goal}: Show cross-filling with \x{pivots NOT on diagonal}, using \x{3 colors}.
...
\x{Pivots will be at}:
\begin{itemize}
\item \textcolor{red}{Pivot 1}: Row 1, Col 2 — \textcolor{red}{Red}
\item \textcolor{blue}{Pivot 2}: Row 2, Col 3 — \textcolor{blue}{Blue}
\item \textcolor{green!50!black}{Pivot 3}: Row 3, Col 1 — \textcolor{green!50!black}{Green}
\end{itemize}

\x{Notice}: NOT (1,1), (2,2), (3,3) — completely \x{off diagonal}!
\end{frame}

\begin{frame}[fragile]{Step 1: First Pivot (Row 1, Col 2) — \textcolor{red}{Red}}
Pick pivot at position \x{(row 1, column 2)}, value 6:
...
\node[draw=red, ultra thick, fit=(m-1-2), inner sep=3pt] {};
\node[right=0.3cm of m, text=red] {\textbf{Pivot 1}};
\node[left=0.3cm of m-1-1, text=red] {\textbf{Row 1}};
\end{tikzpicture}
\end{center}

\vfill
Extract \textcolor{red}{$R_1$}:
$$R_1 = \underbrace{\begin{pmatrix} \textcolor{red}{6} \\ 2 \\ 1 \end{pmatrix}}_{\mathbf{u}_1 = \text{column 2}} \cdot \underbrace{\begin{pmatrix} 0 & 1 & \frac{1}{2} \end{pmatrix}}_{\mathbf{v}_1^T = \text{row 1}/6} = \begin{pmatrix} 0 & 6 & 3 \\ 0 & 2 & 1 \\ 0 & 1 & \frac{1}{2} \end{pmatrix}$$
\end{frame}

\begin{frame}[fragile]{Step 2: After Peeling \textcolor{red}{$R_1$} — \textcolor{red}{Row 1 Eliminated}}
Remainder:
...
\begin{tikzpicture}[scale=0.95]
\matrix (m) [matrix of math nodes, left delimiter=(, right delimiter=), nodes={minimum width=1.1cm, minimum height=0.8cm}] {
  |[fill=gray!40]| 0 & |[fill=gray!40]| 0 & |[fill=gray!40]| 0 \\
  0 & 0 & |[fill=blue!30]| 3 \\
  5 & 0 & \frac{13}{2} \\
};
\node[draw=blue, ultra thick, fit=(m-2-3), inner sep=3pt] {};
\node[right=0.3cm of m, text=blue] {\textbf{Pivot 2}};
\draw[->,red,ultra thick] (m.west) -- ++(-2,0) node[left, text=red] {\textbf{\textcolor{red}{Row 1} is ZERO!}};
\end{tikzpicture}
\end{center}

\vfill
\x{Key}: After peeling \textcolor{red}{$R_1$}, \textcolor{red}{\x{row 1 (the pivot row) became zero}}!

New pivot: \textcolor{blue}{(row 2, col 3)}, value 3
\end{frame}

\begin{frame}[fragile]{Step 3: Extract \textcolor{blue}{$R_2$} — Notice the Zeros!}
From remainder $A_2 = \begin{pmatrix} 0 & 0 & 0 \\ 0 & 0 & 3 \\ 5 & 0 & \frac{13}{2} \end{pmatrix}$, pivot at \textcolor{blue}{(row 2, col 3)}:

\vfill
Extract \textcolor{blue}{$R_2$}:
$$R_2 = \underbrace{\begin{pmatrix} \textcolor{red}{0} \\ \textcolor{blue}{3} \\ \frac{13}{2} \end{pmatrix}}_{\mathbf{u}_2 = \text{column 3}} \cdot \underbrace{\begin{pmatrix} 0 & 0 & 1 \end{pmatrix}}_{\mathbf{v}_2^T = \text{row 2}/3}$$

\vfill
\x{Observe $\mathbf{u}_2$}:
$$\mathbf{u}_2 = \begin{pmatrix} \textcolor{red}{\mathbf{0}} \\ \textcolor{blue}{3} \\ \frac{13}{2} \end{pmatrix} \quad \leftarrow \text{\textcolor{red}{First entry is 0}!}$$

\vfill
\x{Why?} Because \textcolor{red}{row 1} of $A_2$ was already zero (eliminated by $R_1$).

So column 3 of $A_2$ \textcolor{red}{must have 0 in row 1}!
\end{frame}

\begin{frame}[fragile]{Step 4: After Peeling \textcolor{blue}{$R_2$} — \textcolor{blue}{Row 2 Eliminated}}
Remainder after $R_2$:
...
\node[draw=green!50!black, ultra thick, fit=(m-3-1), inner sep=3pt] {};
\node[right=0.3cm of m, text=green!50!black] {\textbf{Pivot 3}};
\draw[->,red,ultra thick] (m.west) -- ++(-2.3,0) node[left, text=red] {\textcolor{red}{\textbf{Row 1}} zero};
\draw[->,blue,ultra thick] (m.west) -- ++(-2.3,-0.4) node[left, text=blue] {\textcolor{blue}{\textbf{Row 2}} zero};
\end{tikzpicture}
\end{center}

\vfill
\x{Now}: \textcolor{red}{Row 1} AND \textcolor{blue}{Row 2} are zero!

Final pivot: \textcolor{green!50!black}{(row 3, col 1)}, value 5
\end{frame}

\begin{frame}[fragile]{Step 5: Extract \textcolor{green!50!black}{$R_3$} — All Pivots Found!}
From remainder $A_3 = \begin{pmatrix} 0 & 0 & 0 \\ 0 & 0 & 0 \\ 5 & 0 & 0 \end{pmatrix}$, pivot at \textcolor{green!50!black}{(row 3, col 1)}:

\vfill
Extract \textcolor{green!50!black}{$R_3$}:
$$R_3 = \underbrace{\begin{pmatrix} \textcolor{red}{0} \\ \textcolor{blue}{0} \\ \textcolor{green!50!black}{5} \end{pmatrix}}_{\mathbf{u}_3 = \text{column 1}} \cdot \underbrace{\begin{pmatrix} 1 & 0 & 0 \end{pmatrix}}_{\mathbf{v}_3^T = \text{row 3}/5}$$

\vfill
\x{Observe $\mathbf{u}_3$}:
$$\mathbf{u}_3 = \begin{pmatrix} \textcolor{red}{\mathbf{0}} \\ \textcolor{blue}{\mathbf{0}} \\ \textcolor{green!50!black}{5} \end{pmatrix} \quad \leftarrow \text{First TWO entries are \textcolor{red}{0}, \textcolor{blue}{0}!}$$

\vfill
\x{Why?} Because \textcolor{red}{row 1} and \textcolor{blue}{row 2} of $A_3$ were both zero.

Column 1 of $A_3$ has \textcolor{red}{0 in row 1}, \textcolor{blue}{0 in row 2}!
\end{frame}

\begin{frame}[fragile]{The Perfect Triangular Structure — 3 Colors!}
Collected columns from cross-filling:
$$U = \begin{pmatrix} \mathbf{u}_1 & \mathbf{u}_2 & \mathbf{u}_3 \end{pmatrix} = \begin{pmatrix} 6 & \textcolor{red}{0} & \textcolor{red}{0} \\ 2 & 3 & \textcolor{blue}{0} \\ 1 & \frac{13}{2} & 5 \end{pmatrix}$$
...
\draw[->,red,ultra thick] (m-1-2.east) -- ++(2.5,0) node[right, text=red] {\textbf{Zero} (row 1 eliminated)};
\draw[->,red,ultra thick] (m-1-3.east) -- ++(2.5,0.3) node[right, text=red] {};
\draw[->,blue,ultra thick] (m-2-3.east) -- ++(2.5,-0.3) node[right, text=blue] {\textbf{Zero} (row 2 eliminated)};
\node[left=0.3cm of m-1-1, text=red] {\textbf{\textcolor{red}{Row 1}}};
\node[left=0.3cm of m-2-1, text=blue] {\textbf{\textcolor{blue}{Row 2}}};
\node[left=0.3cm of m-3-1, text=green!50!black] {\textbf{\textcolor{green!50!black}{Row 3}}};
\end{tikzpicture}
\end{center}

\vfill
\x{Perfect triangular pattern}:
\begin{itemize}
\item $\mathbf{u}_2$ has \textcolor{red}{0 in row 1} (previous pivot row)
\item $\mathbf{u}_3$ has \textcolor{red}{0 in row 1}, \textcolor{blue}{0 in row 2} (all previous pivot rows)
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Visual Summary: Why Independence? — Backward Substitution}
$$U = \begin{pmatrix}
\tikz[baseline=(char.base)]{\node[fill=red!30] (char) {6};} & \tikz[baseline=(char.base)]{\node[fill=red!30] (char) {0};} & \tikz[baseline=(char.base)]{\node[fill=red!30] (char) {0};} \\
2 & \tikz[baseline=(char.base)]{\node[fill=blue!30] (char) {3};} & \tikz[baseline=(char.base)]{\node[fill=blue!30] (char) {0};} \\
1 & \frac{13}{2} & \tikz[baseline=(char.base)]{\node[fill=green!20] (char) {5};}
\end{pmatrix}$$
...
\vfill
\begin{itemize}
\item \textcolor{red}{\textbf{Row 1}}: $\tikz[baseline=(char.base)]{\node[fill=red!30] (char) {$6c_1$};} + \tikz[baseline=(char.base)]{\node[fill=red!30] (char) {$0c_2$};} + \tikz[baseline=(char.base)]{\node[fill=red!30] (char) {$0c_3$};} = 0 \Rightarrow \boxed{c_1 = 0}$

\vfill
\item \textcolor{blue}{\textbf{Row 2}}: $2c_1 + \tikz[baseline=(char.base)]{\node[fill=blue!30] (char) {$3c_2$};} + \tikz[baseline=(char.base)]{\node[fill=blue!30] (char) {$0c_3$};} = 0 + 3c_2 + 0 = 0 \Rightarrow \boxed{c_2 = 0}$

\vfill
\item \textcolor{green!50!black}{\textbf{Row 3}}: $1c_1 + \frac{13}{2}c_2 + \tikz[baseline=(char.base)]{\node[fill=green!20] (char) {$5c_3$};} = 0 + 0 + 5c_3 = 0 \Rightarrow \boxed{c_3 = 0}$
\end{itemize}

\vfill
\x{Triangular pattern} (colored zeros!) allows \x{backward substitution}: solve row 1 $\to$ row 2 $\to$ row 3!
\end{frame}
